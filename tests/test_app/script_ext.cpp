...
void ScriptExt::pressButton(const QString &caption)
{
    ...
    for (i = 0; i < nAttempts; ++i) {
        const QString errMsg = agent->runCodeInGuiThreadSync([&btn, &caption] {
            ...
            return QString("Can not find button with such text %1")
                .arg(caption);
        });
        if (errMsg.isEmpty())
            break;
    ...
    if (i == nAttempts) {
        agent->throwScriptError(
            QString("Can not find button with such text %1").arg(caption));
        return;
    }
    assert(btn != nullptr);

    QString errMsg = agent->runCodeInGuiThreadSyncWithTimeout(
        [btn, agent] {
